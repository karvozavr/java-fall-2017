...
public final class FirstPartTasks {
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
        return albums
                ...
                .filter(number -> number.getValue() > 1).count();
    }
    ...
    public static Optional<Album> minMaxRating(Stream<Album> albums) {
        return albums
                .min(Comparator.comparing(album -> album.getTracks().stream().map(Track::getRating).max(Comparator.naturalOrder()).orElse(0)));
    }

    // Список альбомов, отсортированный по убыванию среднего рейтинга его треков (0, если треков нет)
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums
                .sorted(Comparator.comparing(album -> ((Album) album).getTracks().stream().mapToInt(Track::getRating).average().orElse(0)).reversed())
                .collect(Collectors.toList());
    }
